...
describe('+ emptyDir()', () => {
  ...
  for (const code of ['EACCES', 'EIO']) {
    it(`should throw the original ${code} readdir error`, () => {
      const error = Object.assign(new Error('readdir failed'), { code })
      const { emptyDirSync } = proxyquire('..', {
        '../fs': { readdirSync: () => { throw error } }
      })

      assert.throws(() => emptyDirSync(TEST_DIR), err => err === error)
    })
  }
