...
async function createSymlink (srcpath, dstpath, type) {
  ...
  if (stats && stats.isSymbolicLink()) {
    ...
    // Following the existing symlink fails if it's broken; in that case fall
    // through so fs.symlink reports EEXIST, rather than leaking the stat ENOENT.
    let dstStat
    try {
...
function createSymlinkSync (srcpath, dstpath, type) {
  ...
  if (stats && stats.isSymbolicLink()) {
    ...
    // Following the existing symlink fails if it's broken; in that case fall
    // through so fs.symlinkSync reports EEXIST, rather than leaking the stat ENOENT.
    let dstStat
    try {
