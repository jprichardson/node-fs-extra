...
describe('fse-ensure-symlink', function () {
  ...
  function fileSuccess (args, fn) {
    ...
    it(should, function (done) {
      var callback = function (err) {
        ...
        var fileContent = fs.readFileSync(relative.toCwd, 'utf8')
        var dstDir = path.dirname(dstpath)
        var dstBasename = path.basename(dstpath)
        expect(fs.readdirSync(dstDir)).to.contain(dstBasename)
        expect(fs.lstatSync(dstpath).isSymbolicLink()).to.equal(true)
  ...
  function dirSuccess (args, fn) {
    ...
    it(should, function (done) {
      var callback = function (err) {
        ...
        var dstDir = path.dirname(dstpath)
        var dstBasename = path.basename(dstpath)
        var dstpathContents = fs.readdirSync(dstpath)
        var srcpathContents = fs.readdirSync(relative.toCwd)
        expect(fs.readdirSync(dstDir)).to.contain(dstBasename)
        expect(fs.lstatSync(dstpath).isSymbolicLink()).to.equal(true)
        expect(dstpathContents).to.deep.equal(srcpathContents)
        return done()
      }
      args.push(callback)
      return fn.apply(null, args)
  ...
  function dirBrokenSync (args, fn) {
    ...
    it(should, function () {
      ...
      expect(fs.lstatSync(dstpath).isSymbolicLink()).to.equal(true)
      expect(function () {
        return fs.readdirSync(dstpath, 'utf8')
      }).to.throw
    })
  }
