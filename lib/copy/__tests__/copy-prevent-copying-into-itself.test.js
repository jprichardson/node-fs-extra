...
function testError (src, dest, done) {
  fs.copy(src, dest, err => {
    assert.strictEqual(err.message, `Cannot copy directory '${src}' into itself '${dest}'`)
    done()
  })
}
...
describe('+ copySync() - prevent copying into itself', () => {
  ...
  beforeEach(done => {
    TEST_DIR = path.join(os.tmpdir(), 'fs-extra', 'copy-sync-prevent-infinite-recursion')
    src = path.join(TEST_DIR, 'src')
    fs.mkdirsSync(src)

    // ensureFileSync creates required parent dirs for us:)
  ...
  describe('> when source is a directory', () => {
    ...
    describe('>> when dest is a symlink', () => {
      ...
      it('should throw an error when resolved dest path is a subdir of src', done => {
        ...
        fs.copy(src, destLink, err => {
          assert.strictEqual(err.message, `Cannot copy directory '${src}' into itself '${resolvedDestPath}'`)
          done()
        })
      })
  ...
  describe('> when source is a symlink', () => {
    describe('>> when dest is a directory', () => {
      ...
      it('should copy the directory successfully when resolved src path is a subdir of dest', done => {
        const srcLink = path.join(TEST_DIR, 'src_symlink')
        const resolvedSrcPath = path.join(TEST_DIR, 'dest', 'some', 'nested', 'src')
        ...
        const srclen = klawSync(resolvedSrcPath).length
        assert(srclen > 2)
        const destlenBefore = klawSync(dest).length
    ...
    describe('>> when dest is a symlink', () => {
      ...
      it('should copy the directory correctly when resolved src path is a subdir of resolved dest path', done => {
        const srcInDest = path.join(TEST_DIR, 'dest', 'some', 'nested', 'src')
        const srcLink = path.join(TEST_DIR, 'src_symlink')
