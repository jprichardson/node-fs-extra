...
function testSuccess (src, dest, done) {
  ...
  fs.copy(src, dest, err => {
    ...
    assert.equal(destlen, srclen, 'src and dest length should be equal')
    ...
    assert.equal(o0, dat0, 'file contents matched')
    assert.equal(o1, dat1, 'file contents matched')
    assert.equal(o2, dat2, 'file contents matched')
    assert.equal(o3, dat3, 'file contents matched')
    done()
  })
}
...
function testError (src, dest, done) {
  fs.copy(src, dest, err => {
    assert.equal(err.message, `Cannot copy directory '${src}' into itself '${dest}'`)
    done()
  })
}
...
describe('+ copy() - prevent copying into itself', () => {
  ...
  describe('> when source is a file', () => {
    it(`should copy the file successfully even when dest is a subdir of src`, done => {
      ...
      fs.copy(srcFile, destFile, err => {
        ...
        assert.equal(out, dat0, 'file contents matched')
        done()
      })
    })
  })
  ...
  describe('> when source is a directory', () => {
    describe('>> when dest is a directory', () => {
      ...
      it(`should throw error when dest is 'src/dest'`, done => {
        dest = path.join(TEST_DIR, 'src', 'dest')
        return testError(src, dest, done)
      })

      it(`should throw error when dest is 'src/src_dest'`, done => {
        dest = path.join(TEST_DIR, 'src', 'src_dest')
        return testError(src, dest, done)
      })

      it(`should throw error when dest is 'src/dest_src'`, done => {
        dest = path.join(TEST_DIR, 'src', 'dest_src')
        return testError(src, dest, done)
      })

      it(`should throw error when dest is 'src/dest/src'`, done => {
        dest = path.join(TEST_DIR, 'src', 'dest', 'src')
        return testError(src, dest, done)
      })
    })
    ...
    describe('>> when dest is a symlink', () => {
      it('should not copy and return when dest points exactly to src', done => {
        ...
        fs.copy(src, destLink, err => {
          ...
          assert.equal(srclenAfter, srclenBefore, 'src length should not change')

          const link = fs.readlinkSync(destLink)
          assert.equal(link, src)
          done()
        })
      })

      it('should throw an error when resolved dest path is a subdir of src', done => {
        const destLink = path.join(TEST_DIR, 'dest-symlink')
        ...
        // make symlink that points to a subdir in src
      ...
      it('should copy the directory successfully when src is a subdir of resolved dest path', done => {
        ...
        fs.copy(srcInDest, destLink, err => {
          ...
          assert.equal(destlenAfter, destlenBefore + srclen, 'dest length should be equal to old length + copied legnth')
          ...
          assert.equal(o0, dat0, 'files contents matched')
          assert.equal(o1, dat1, 'files contents matched')
          assert.equal(o2, dat2, 'files contents matched')
          assert.equal(o3, dat3, 'files contents matched')
          done()
        })
      })
    })
  ...
  describe('> when source is a symlink', () => {
    describe('>> when dest is a directory', () => {
      it('should not copy and return when resolved src path points to dest', done => {
        const srcLink = path.join(TEST_DIR, 'src-symlink')
        fs.symlinkSync(src, srcLink, 'dir')

        dest = path.join(TEST_DIR, 'src')
      ...
      it('should throw an error when dest not exist and is a subdir of resolved src path', done => {
        const srcLink = path.join(TEST_DIR, 'src-symlink')
        fs.symlinkSync(src, srcLink, 'dir')

        dest = path.join(TEST_DIR, 'src', 'some', 'nested', 'dest')
      ...
      it('should throw an error when dest exists and is a subdir of resolved src path', done => {
        const srcLink = path.join(TEST_DIR, 'src-symlink')
        fs.symlinkSync(src, srcLink, 'dir')
      ...
      it('should throw an error when resolved src path is a subdir of dest', done => {
        dest = path.join(TEST_DIR, 'dest')
        ...
        fs.copy(srcLink, dest, err => {
          assert.equal(err.message, `'${dest}' already exists and contains '${srcLink}'`)
          done()
        })
      })
    ...
    describe('>> when dest is a symlink', () => {
      it('should not copy and return when resolved dest path is exactly the same as resolved src path', done => {
        const srcLink = path.join(TEST_DIR, 'src-symlink')
        fs.symlinkSync(src, srcLink, 'dir')
        const destLink = path.join(TEST_DIR, 'dest-symlink')
        fs.symlinkSync(src, destLink, 'dir')
        ...
        fs.copy(srcLink, destLink, err => {
          ...
          assert.equal(destlenAfter, destlenBefore, 'dest length should not change')

          const srcln = fs.readlinkSync(srcLink)
          assert.equal(srcln, src)
          const destln = fs.readlinkSync(destLink)
          assert.equal(destln, src)
          done()
        })
      })

      it('should not copy and throw error when resolved dest path is a subdir of resolved src path', done => {
        const srcLink = path.join(TEST_DIR, 'src-symlink')
        fs.symlinkSync(src, srcLink, 'dir')
      ...
      it('should throw an error when resolved src path is a subdir of resolved dest path', done => {
        const srcInDest = path.join(TEST_DIR, 'dest', 'some', 'nested', 'src')
        const srcLink = path.join(TEST_DIR, 'src-symlink')
        // put some stuff in resolved src path
        // fs.copySync(src, srcInDest)
        ...
        fs.copy(srcLink, destLink, err => {
          assert.equal(err.message, `'${destLink}' already exists and contains '${srcLink}'`)
          done()
        })
      })
    })
