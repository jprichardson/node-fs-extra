...
describe('+ copy() - without checking paths', () => {
  let TEST_DIR, src

  beforeEach(done => {
    TEST_DIR = path.join(os.tmpdir(), 'fs-extra', 'copy-without-checking-paths')
  ...
  describe('> when source is a file', () => {
    it('should copy the file successfully even if dest parent is a subdir of src', done => {
      ...
      fs.copy(srcFile, destFile, { checkPathsBeforeCopying: false }, err => {
        assert.ifError(err)
  ...
  describe('> when source is a directory', () => {
    ...
    describe('>> when dest is a symlink', () => {
      it('should copy the directory successfully when src is a subdir of resolved dest path', done => {
        ...
        assert(srclen > 2)
        fs.copy(srcInDest, destLink, { checkPathsBeforeCopying: false }, err => {
          assert.ifError(err)
  ...
  describe('> when source is a symlink', () => {
    describe('>> when dest is a directory', () => {
      it('should error when resolved src path points to dest', done => {
        ...
        fs.copy(srcLink, dest, { checkPathsBeforeCopying: false }, err => {
          assert(err)
    ...
    describe('>> when dest is a symlink', () => {
      it('should error when resolved dest path is a subdir of resolved src path', done => {
        ...
        fs.copy(srcLink, destLink, { checkPathsBeforeCopying: false }, err => {
...
function testSuccess (src, dest, done) {
  ...
  assert(srclen > 2)
  fs.copy(src, dest, { checkPathsBeforeCopying: false }, err => {
    assert.ifError(err)
