...
function copy (src, dest, options, cb) {
  ...
  stat(src, (err, st) => {
    ...
    if (st.isDirectory()) {
      if (options.filter) return applyFilterOnDir()
      return onDir(st)
    } else if (st.isFile() || st.isCharacterDevice() || st.isBlockDevice()) {
      if (options.filter) return applyFilterOnFile()
      return onFile(st)
    ...
    function applyFilterOnDir () {
      if (options.filter instanceof RegExp) {
        ...
        if (!options.filter.test(src)) {
          if (options.noRecurseOnFailedFilter) return cb()
  ...
  function checkDest (callback) {
    mkdirp(path.dirname(dest), err => {
      if (err) return callback(err)
      fs.readlink(dest, (err, resolvedDestPath) => {
        if (err) {
          if (err.code === 'ENOENT') return callback(null, DEST_NOENT)
          // dest exists but is not a link, Windows throws UNKNOWN error
          if (err.code === 'EINVAL' || err.code === 'UNKNOWN') return callback(null, DEST_EXISTS)
          return callback(err)
        }
        // dest exists and is a link
        return callback(null, resolvedDestPath)
      })
    })
  }
  ...
  function onLink () {
    fs.readlink(src, (err, resolvedSrcPath) => {
      ...
      checkDest((err, resolvedDestPath) => {
        if (err) return cb(err)
